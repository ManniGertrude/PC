...
Data = pd.read_csv(f'{path}\\Daten\\Mikroskop_WS_24-25.csv', sep=",",header=0, names=Anteile)
temp = 0
for j in Anteile:
...
fit(lin, Anteil[:9], Mittelwertliste[i][:9], Data[10:], Sigma[i][:9], 'navy', True)
plt.errorbar(Anteil, Mittelwertliste[i], xerr=0.01, yerr=Sigma[i], color = 'navy', capsize=3, linestyle='none')
Plotparams('WS_24-25', f'{len(Data[j])} Messungen {Bezeichnung[i]}') # Dateien[i]

# Alle Semester in einen Plot übereinander gelegt
fit(lin, Anteil[:9], Mittelwertliste[i][:9], None, Sigma[i][:9], 'navy', False)
plt.errorbar(Anteil, Mittelwertliste[i], xerr=0.01, yerr=Sigma[i], color='navy', capsize=3, linestyle='none', label=Bezeichnung[i])
X_Y_Spline = make_interp_spline(Anteil[9:], Mittelwertliste[i][9:])
Y_ = X_Y_Spline(xWerte)
plt.plot(xWerte, Y_, color='navy', alpha=0.8)
...
Zieldata = pd.read_csv(f'{path}\\Daten\\Mikroskop_WS_24-25.csv', sep=",",header=0, names=Anteile)
